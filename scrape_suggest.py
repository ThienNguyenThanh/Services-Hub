...

{
    ...
    'synonyms': [ {
        'type': 'noun',
        'meaning': 'journey, excursion',
        'synonymss': ['curise', 'foray', ...]
    }, 
    {
        'type': 'noun',
        'meaning': 'journey, excursion',
...
# synonyms = []
# for re in diffent_types_bar:
    # if len(re.find_all('em')) > 0 and len(re.find_all('strong')) > 0:
    #     word_types = re.find_all('em')
    #     meanings = re.find_all('strong')
    #     for i in range(len(word_types)):
    #         each_syn = {'type': word_types[i].text, 'meaning': meanings[i].text}
